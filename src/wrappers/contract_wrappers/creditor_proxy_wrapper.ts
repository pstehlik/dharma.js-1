...
export class CreditorProxyContract extends BaseContract {
    public cancelDebtOffer = {
        async sendTransactionAsync(
            ...
            )(commitmentAddresses, commitmentValues, termsContractParameters, txDataWithDefaults);
            return txHash;
        },
        ...
        async getABIEncodedTransactionData(
            ...
            const abiEncodedTransactionData = await promisify<string>(
                self.web3ContractInstance.cancelDebtOffer.getData,
                self.web3ContractInstance,
            )(commitmentAddresses, commitmentValues, termsContractParameters, txDataWithDefaults);
    ...
    public fillDebtOffer = {
        ...
        async getABIEncodedTransactionData(
            ...
            const txDataWithDefaults = await self.applyDefaultsToTxDataAsync(txData);
            const abiEncodedTransactionData = await promisify<string>(
                self.web3ContractInstance.fillDebtOffer.getData,
                self.web3ContractInstance,
            )(
                creditor,
                orderAddresses,
    ...
    public debtOfferCancelled = {
        async callAsync(index_0: string, defaultBlock?: Web3.BlockParam): Promise<boolean> {
            const self = this as CreditorProxyContract;
            const result = await promisify<boolean>(
